...
function table(data) {
    ...
    CreateTable.setAttribute("style", "width:100%;");
    ...
    var brand1 = document.createElement("th");
    brand1.innerText = ("Brand");

    var prise1 = document.createElement("th");
    prise1.innerText = ("Price");

    var quntity1 = document.createElement("th");
    quntity1.innerText = ("Quntity");
    ...
    tr.appendChild(brand1);
    tr.appendChild(prise1);
    tr.appendChild(quntity1);
    ...
    for (var product of data) {
        ...
        var idn = document.createElement("td");
        idn.innerText = (product.id);

        var namen = document.createElement("td");
        namen.innerText = (product.title);

        var brandn = document.createElement("td");
        brandn.innerText = (product.brand);

        var prisen = document.createElement("td");
        prisen.innerText = (product.price);
        p += ((product.price) * 1);

        var qun = document.createElement("td");
        qun.innerText = ("1");

        var tot = document.createElement("td");
        tot.innerText = (product.price);
        tp += ((product.price) * 1);

        trnew.appendChild(idn);
        trnew.appendChild(namen);
        trnew.appendChild(brandn);
        trnew.appendChild(prisen);
        trnew.appendChild(qun);
        trnew.appendChild(tot);

        CreateTable.appendChild(trnew);
    }
    CreateTable.appendChild(trnew);

    var totalprice1 = document.createElement("tr");

    var totalp1 = document.createElement("td");
    totalp1.setAttribute("colspan", "3");
    totalprice1.appendChild(totalp1);

    var totalp2 = document.createElement("td");
    totalp2.innerText = (p);
    totalprice1.appendChild(totalp2);

    var totalp3 = document.createElement("td");
    totalprice1.appendChild(totalp3);

    var totalp = document.createElement("td");
    totalp.innerText = (tp);
    totalprice1.appendChild(totalp);

    CreateTable.appendChild(totalprice1);

    var chackout = document.createElement("button");
    chackout.innerText = ("Chackout")
    chackout.setAttribute("onclick", "raj()")


    main.appendChild(CreateTable);
    main.appendChild(chackout);
}

let data = {
    headache: {
        Product: {
            medicine: ["migrakal"], Syrupe: ["Pathyakshadhatryadi Kashaya"], Cream: ["Go Mygraine"]
        }, Therapy: ["fingre press", "aspirin"], HomeRemedy: ["Drinking Water", "Relaxing With Yoga", "Avoid strong smells"], Yoga: ["yoga Asanas to relieve headaches", "Marjariasana", "Ardha Pincha Mayurasana", "Balasana"], DoctorConsult: { chat: { rajthakur: "8827142011", devkanaya: "6352425142", jyoti: "7583546372" }, call: { rajthakur: "8827142011", devkanaya: "6352425142", jyoti: "7583546372" } }
    }
}



Create table Order(Id int not null primary key auto_increment, OrderDate date, OrderAmount int not null, DeliveryAddress varchar(300) not null, PersonContactDetails varchar(10) not null, PaymentMod varchar(50) not null, PandingOrder boolean not null bydefault(false), CancleOrder boolean not null bydefault(false), CompleteOrder boolean not null bydefault(false), UserId int, foreign key(UserId) references User(Id));


create table OrderItems(id int NOT NULL PRIMARY KEY AUTO_INCREMENT,quantity int not null,ProductId int,OrderId int,foreign key(productid) references product(id),foreign key(orderid) references OrderDetails(id));


CREATE TABLE doctor ( id INT AUTO_INCREMENT PRIMARY KEY, Name VARCHAR(50) NOT NULL, gender ENUM('Male', 'Female', 'Other') NOT NULL, specialization VARCHAR(100), email VARCHAR(100) UNIQUE, phone_number VARCHAR(10), address VARCHAR(255), graduationDetails varchar(200) not null);



CREATE TABLE deliveryboy ( deliveryboy_id INT PRIMARY KEY AUTO_INCREMENT, name VARCHAR(50) NOT NULL, email VARCHAR(100) UNIQUE NOT NULL, phone_number VARCHAR(15), vehicle_number VARCHAR(20), Address varchar(50) not null);